...
import {getJ2Breakfast, getJ2Dinner, getJ2Food, getJ2Lunch, getJCLBreakfast, getJCLDinner, getJCLLunch } from '@/lib/appwrite'
...
//   const {data: posts, refetch} = useAppwrite(getAllFoodData, []);
  ...
  const {data: breakfast} = useAppwrite(getJCLBreakfast, []);
  const {data: lunch} = useAppwrite(getJCLLunch, []);
  const {data: dinner} = useAppwrite(getJCLDinner, []);
